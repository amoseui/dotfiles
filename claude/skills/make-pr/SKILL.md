...
- 문장 톤: **`키워드: 설명`** 형태로 짧게. 굵은 키워드 뒤에 짧은 설명을 붙이고, 가능하면 체언(명사형)으로 종결한다. 긴 서술형 문장 대신 축약된 단문을 쓴다.
  - 예: `- **검색·필터**: 제목·작성자·라벨로 실시간 필터` / `- **주소**: /patches로 정리, 구주소 자동 이동`
  - 지양: `- 방문자가 원하는 항목을 찾기 쉽도록 검색과 필터 기능을 추가하였다.`
