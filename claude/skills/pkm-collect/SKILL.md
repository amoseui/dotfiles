...
description: |
  ...
  "6-agents/notes"에 노트로 합성하고 daily note에 백링크를 건다.
...
> **맥스튜디오 Hermes의 pkm-collect**가 담당하는 단일 경로다. 이 스킬은 그
...
- ★ **Claude가 자동 생성하는 노트는 사람 노트와 분리한다**: 노트 본체는 항상 **`6-agents/notes/`** 안에만 만든다. 사람 존 폴더(`1-inbox/`, `3-projects/`, `4-areas/`, `2-journal/`)에 직접 쓰지 않는다 (daily note의 지정 섹션 편집은 예외).
...
1. 세부가 필요하면 해당 세션 `transcript_path`를 **타깃 grep/jq로만** 부분 조회(전체 읽기 금지).
   - **superpowers 작업 문서 참고**: 작업항목의 `project` repo 루트에 `docs/superpowers/specs/*.md`(brainstorming 설계)·`docs/superpowers/plans/*.md`(writing-plans 구현 계획)가 있으면, 이 작업과 **관련된 것만** 읽어 노트의 `# 목적`·`# 기술적 고려사항`(왜 했나·핵심 의사결정)을 보강한다. gitignore된 로컬 산출물이라 없을 수 있으니, 있을 때만 graceful하게 쓰고 없으면 조용히 건너뛴다. 참고한 문서는 노트 `# 참고`에 repo-상대경로로 출처를 남긴다.
     ...
     ```
2. `references/note-template.md` 구조로 **`/Users/amoseui/Obsidian/amoseui/6-agents/notes/<한글 자연어 제목>.md`**를 **Write 도구로** 생성.
   - 폴더가 없으면 만든다(`6-agents/notes/`).
...
3. 오늘 daily note `/Users/amoseui/Obsidian/amoseui/2-journal/daily/$(date +%Y-%m-%d).md`에 백링크 추가:
...
4. **obsidian-history 적용**: daily note(사람 파일)를 편집했으므로 `[[obsidian-history]]` 규칙에 따라 daily note의 `## History`와 감사 로그(`6-agents/state/changelog/YYYY-MM.md`)에 변경을 기록한다. `6-agents/notes/`의 생성 노트는 AI 전용 영역이므로 노트 내 `## History`(최초 생성)만 남기고 감사 로그에는 중복 기록하지 않는다.
...
- Claude 생성 노트는 반드시 `6-agents/notes/` 아래에만 둔다(사람 노트 영역 오염 금지).
