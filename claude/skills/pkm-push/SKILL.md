...
description: |
  ...
  (6-agents/inbox/codex-claude/)에 밀어넣는다. 실제 노트 합성·daily 백링크는 이 머신이
...
- 인박스: `{vault}/6-agents/inbox/codex-claude/`. 파일명 `{hostname}-{YYYY-MM-DD-HHMM}.json`.
...
INBOX="$VAULT/6-agents/inbox/codex-claude"
mkdir -p "$INBOX"
HOST=$(hostname -s)
...
### 3. 동기 확인 + 보고
- 파일이 `{vault}/6-agents/inbox/codex-claude/`에 생성됐는지 `ls`로 확인.
