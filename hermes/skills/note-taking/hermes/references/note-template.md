...
> 이 템플릿의 노트는 config.yaml의 `note_dir`(기본 `6-agents/notes/`) 또는
> `media_dir`(책/영화, 기본 `6-agents/media/`)에만 생성한다.
> `6-agents/notes/`는 Claude/Codex 등 다른 에이전트와 공유하는 폴더다(author frontmatter로 구분) — 섞지 않는 대상은 사람이 쓴 노트다.
