...
- 기존 노트의 성격을 먼저 분류: ① 외부 소스 클리핑/하이라이트(Readwise 등) → `raw/`로 흡수 후 컴파일. ② 잡다 메모 → `6-agents/notes/`로 단순 이동. ③ 이미 정리된 지식 → 소스로 삼아 재종합.
...
원본 폴더는 **보존**(읽기만), `5-wiki/raw/articles|books/`로 복사 + raw frontmatter 부착.

```python
import hashlib, re, os
src_dir = "<원본 폴더>"; raw_dir = "<vault>/5-wiki/raw/articles"; os.makedirs(raw_dir, exist_ok=True)
