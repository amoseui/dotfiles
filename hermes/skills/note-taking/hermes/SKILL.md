...
- **vault root 경로 주의** — 이 vault의 실제 root는 `.../Obsidian/amoseui/amoseui`(amoseui 두 번). Claude Code의 `pkm` 스킬은 한 단계 위(`.../Obsidian/amoseui`)로 잘못 적혀 있으니 그 값을 그대로 베끼지 말 것. 정확한 root는 `obsidian-history` 스킬과 이 스킬의 config.yaml 기준.
