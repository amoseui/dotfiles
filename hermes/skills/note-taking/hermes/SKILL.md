...
vault는 4개 존으로 나뉜다. Hermes는 이 중 **에이전트 존**에 노트를 쓰고, **지식 존**(wiki)을 운영한다.
...
- **예외**: 감사 로그 자체를 수정할 때는 거기에 중복 기록하지 않는다(파일 내 History만).

### 3) 예외: cron 자동 반복 기입
cron으로 도는 자동 반복 작업(아침/밤 daily 갱신 등)이 넣는 반복 기입은 파일 내 `## History`에 쌓지 않고 감사 로그(`history_log`)에만 남긴다. 매일 반복되는 자동 기입까지 `## History`에 쌓이면 노트 본문이 금방 지저분해지기 때문이다. 사람이 직접 요청한 편집이나 1회성 변경은 기존대로 두 곳 모두에 기록한다.

### 적용 범위
- 대상: `{vault}/` 하위 모든 `.md`
- 새 파일 최초 생성 시 History 첫 항목은 "최초 생성 (hermes)".
- **제외**: cron 등 자동 반복 작업(아침/밤 daily 갱신 등)의 반복 기입은 파일 내 `## History`에 쌓지 않고 감사 로그에만 남긴다(위 3) 참고).
...
- **config 값 파싱은 YAML 파서로** — config.yaml 값에 인라인 주석(`키: 값  # 설명`)을 쓰면 셸 `grep|sed` 같은 단순 파서가 주석까지 값으로 잡아 깨진 경로의 폴더를 만든다(이 세션에서 실제 발생 — 당시 `note_dir` 값에 주석이 섞여 들어감). 값 검증/사용은 `read_file`로 YAML을 읽어 처리하고, config.yaml에는 인라인 주석을 두지 않는다(설명은 키 위 줄에).
