...
| Git workspace | `/Users/amoseui/Workspace/github` (recent_days=30) |
| 캘린더 | prenine 계정 (scripts via daily-notes-automation) |
| 메일 | amoseui+prenine 별표 + 받은편지함 |
...
VAULT="/Users/amoseui/Obsidian/amoseui/amoseui/Retrospective/1. Daily"
...
```
오늘 일정. 이번 주 전체를 원하면 google_api.py를 prenine HERMES_HOME으로 직접 호출:
```bash
HERMES_HOME=~/.hermes/google-accounts/prenine python3 \
...
## TASK F: Git 로컬 상태
`/Users/amoseui/Workspace/github` 아래 최근 30일 내 커밋 있는 repo를 탐색해 worktree 상태 확인:
```bash
setopt null_glob 2>/dev/null; shopt -s nullglob 2>/dev/null
WORKSPACE="/Users/amoseui/Workspace/github"; RECENT_DAYS=30
...
## TASK G: GitHub PR (gh)
`gh`가 인증돼 있으면(이 맥은 amoseui 로그인됨) 관심 repo의 내 PR + 리뷰 대기 조회:
```bash
...
- 캘린더=prenine, 메일=amoseui+prenine. gh는 amoseui 로그인.
