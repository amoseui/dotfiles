...
def main(argv=None):
    ...
    # vault 기준 노트 인덱스 디렉터리(중복 감지용). 여러 세그먼트 경로 지원(예: "6-agents/notes").
    ...
    # 동기 vault 인박스: 보조 머신(맥북)이 pkm-push로 밀어넣은 digest JSON들을 함께 합친다.
    ...
    # 보조 머신(맥북) 인박스 digest 합치기: pkm-push가 밀어넣은 JSON들의 sessions를
    ...
    # 여러 세그먼트 상대경로(예: "6-agents/notes")를 안전하게 결합
